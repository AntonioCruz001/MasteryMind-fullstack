...
export default function Flashcards() {
    ...
    const handleReview = async (flashcardId, result) => {
        try {
            await api.post(`/subjects/${subjectId}/flashcards/${flashcardId}/review`, { result });
            setFlashcards((prev) => {
                const cardIndex = prev.findIndex(c => c.id === flashcardId);
                if (cardIndex === -1) return prev;
                const updatedCard = {...prev[cardIndex], is_used_today : true};
                const filtered = prev.filter(c => c.id !== flashcardId);
                return [...filtered, updatedCard];
            });
        } catch (err) {
            console.log('Erro ao registrar revisão:', err);
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div
                                        ...
                                        className="min-h-[140px] p-5 bg-gray-50 hover:bg-teal-50/30 rounded-xl border border-gray-200 flex flex-col justify-between cursor-pointer transition-all select-none"
                                    >
